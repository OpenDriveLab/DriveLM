...
def parse_arguments():
    ...
    path_group.add_argument(
        ...
        type=str,
        required=True,
        default="output/path/of/graph/vqas",
        help="Output directory for the vqa-graph",
    )
    path_group.add_argument(
        "--output-graph-examples-directory",
        type=str,
        default="output/path/of/graph/vqas",
    ...
    img_group.add_argument("--original-fov", type=float, default=110, help="Original field of view")
...
def convert_carla_to_nuscenes_and_save(args, carla_file_content):
    ...
    """
    with open("llava_format.json", "w", encoding="utf-8") as f:
        json.dump(carla_file_content, f, indent=4)
    ...
    for path, tick in zip(carla_file_content["image_paths"], carla_file_content["llava_format"]):
        ...
        town_name, _, scenario_name, route_number, _, image_number = path.split("/")[-6:]
        route_number = route_number.split("_")[0] + "_" + route_number.split("_")[1]
        image_number = image_number.replace(".jpg", "")

        save_dir = os.path.join(args.output_graph_directory, town_name, scenario_name, route_number)

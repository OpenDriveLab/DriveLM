...
class ScenarioLogger:
    ...
    def fetch_bg_state(self):
        ...
        for vehicle in self.bg_vehicles:
            ...
            positions.append(
                np.array([[[vehicle.get_location().x, vehicle.get_location().y]]])
            )
            yaws.append(np.array([[[vehicle.get_transform().rotation.yaw]]]))
            velocities.append(
                np.array([[[vehicle.get_velocity().x, vehicle.get_velocity().y]]])
            )
            extents.append(
                np.array(
                    [
    ...
    def log_step(self, route, ego_control=None):
        ...
        if not self.ego_pitch:
            self.ego_orientation = self.ego_vehicle.get_transform()
            self.ego_pitch = np.array(
                [[[np.radians(self.ego_orientation.rotation.pitch)]]]
            )

        if not self.ego_roll:
            self.ego_orientation = self.ego_vehicle.get_transform()
            self.ego_roll = np.array(
                [[[np.radians(self.ego_orientation.rotation.roll)]]]
            )
        ...
        if len(self.bg_vehicles) > 0 and self.bg_color is not None:
            state = {
                ...
                "extent": np.concatenate(
                    [self.ego_extent, self.bg_extent], axis=1
                ).tolist(),
                "id": np.concatenate([self.ego_id, self.bg_id], axis=1).tolist(),
                "type": np.concatenate([self.ego_type, self.bg_type], axis=1).tolist(),
                "color": np.concatenate(
                    [self.ego_color, self.bg_color], axis=1
                ).tolist(),
                "height": np.concatenate(
                    [self.ego_height, self.bg_height], axis=1
                ).tolist(),
                "pitch": np.concatenate(
                    [self.ego_pitch, self.bg_pitch], axis=1
                ).tolist(),
    ...
    def route_as_boxes(self, route):
        ...
        for i, midpoint in enumerate(midpoints):
            ...
            start_bounding_box = carla.Location(

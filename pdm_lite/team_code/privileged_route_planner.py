...
class PrivilegedRoutePlanner(object):
    ...
    def extend_lane_shift_transition_for_yield_to_emergency_vehicle(
        self, shift_to_left_lane, previous_shift_end_index
    ):
        """
    ...
    def extend_lane_shift_transition_for_hazard_at_side_lane(
        self, last_bicycle, previous_shift_end_index
    ):
        """
    ...
    def shift_route_smoothly(
        ...
        Args:
            ...
            lane_transition_factor (float): A factor between 0 and 1 that controls the amount of shift towards
                the neighboring lane. A value of 1.0 means the route will be shifted to the center of the
                neighboring lane, while a value of 0.0 means the route will stay in the center of the current lane.
        """
    ...
    def shift_route_for_invading_turn(self, first_cone, last_cone, lateral_offset):
        ...
        first_cone_index = self.get_closest_route_index(
            self.route_index, first_cone.get_location()
        )
        # Begin 10 meters for the search after the first cone.
        last_cone_index = self.get_closest_route_index(
            first_cone_index + 10 * self.points_per_meter, last_cone.get_location()
        )
    ...
    def shift_route_around_actors(
        ...
        Args:
            ...
            transition_length (int): The length of the transition in waypoints.
            lane_transition_factor (float): A factor between 0 and 1 that controls the amount of shift towards the
                neighboring lane. A value of 1.0 means the route will be shifted to the center of the neighboring lane,
                while a value of 0.0 means the route will stay in the center of the current lane.
    ...
    def setup_route(
        self, global_plan, carla_world, carla_map, starts_with_parking_exit, vehicle_loc
    ):
        """
    ...
    def compute_speed_limits(self, carla_map):
        ...
        for i, loc in enumerate(self.route_points):
            if (
                i % self.speed_limit_waypoints_spacing_check == 0
            ):  # Calculate for waypoints every 5 m for efficiency
                _, min_idx = tree.query(loc, k=1)
                speed_limit = (
                    map_speed_limits[min_idx] / 3.6

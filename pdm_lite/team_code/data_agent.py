...
class DataAgent(AutoPilot):
    ...
    def tick(self, input_data):
        ...
        w, h = bev_semantics["bev_semantic_classes"].shape
        cropped = bev_semantics["bev_semantic_classes"][
            int(h / 4) : int(3 * h / 4), int(w / 4) : int(3 * w / 4)
        ]
        bev_semantics["bev_semantic_classes"] = cropped

        w, h = bev_semantics_augmented["bev_semantic_classes"].shape
        cropped = bev_semantics_augmented["bev_semantic_classes"][
            int(h / 4) : int(3 * h / 4), int(w / 4) : int(3 * w / 4)
        ]
    ...
    def get_bounding_boxes(self, lidar=None):
        ...
        left_decreasing_lane_id = (
            left_wp is not None
            and left_wp.lane_id < ego_wp.lane_id
            or right_wp is not None
            and right_wp.lane_id > ego_wp.lane_id
        )
        ...
        while True:
            flag = (
                ego_wp.lane_id > 0
                and left_decreasing_lane_id
                or ego_wp.lane_id < 0
                and not left_decreasing_lane_id
            )
            if is_opposite:
                flag = not flag
            ...
            if (
                ...
                and ego_wp.lane_id * wp.lane_id < 0
            ):
                break
            ...
            while is_road:
                ...
                if lane_wp is None:
                    ...
                    if (
                        ...
                        and direction == ego_lane_direction
                    ):
                ...
                ego_lane_number = abs(
                    ego_wp.lane_id - lane_id_left_most_lane_same_direction
                )
        ...
        affected_by_vehicle_10, aff_vehicle_id_10, aff_vehicle_dis_10 = (
            self._vehicle_obstacle_detected(vehicle_list, 10)
        )
        if affected_by_vehicle_10:
            hazard_detected_10 = True

        hazard_detected_15 = False
        affected_by_vehicle_15, aff_vehicle_id_15, aff_vehicle_dis_15 = (
            self._vehicle_obstacle_detected(vehicle_list, 15)
        )
        if affected_by_vehicle_15:
            hazard_detected_15 = True

        hazard_detected_20 = False
        affected_by_vehicle_20, aff_vehicle_id_20, aff_vehicle_dis_20 = (
            self._vehicle_obstacle_detected(vehicle_list, 20)
        )
        if affected_by_vehicle_20:
            hazard_detected_20 = True

        hazard_detected_40 = False
        affected_by_vehicle_40, aff_vehicle_id_40, aff_vehicle_dis_40 = (
            self._vehicle_obstacle_detected(vehicle_list, 40)
        )
            ...
            if (
                ...
                if vehicle.id != self._vehicle.id:
                    ...
                    if vehicle_wp.is_junction:
                        ...
                        distance_to_junction = next_lane_wps[
                            0
                        ].transform.location.distance(vehicle_wp.transform.location)
                    else:
                    ...
                    if vehicle_wp.road_id == ego_wp.road_id:
                        ...
                        if not same_direction_as_ego:
                            lane_relative_to_ego += (
                                remove_lanes_for_lane_relative_to_ego
                    ...
                    light_state_bin_pos = [
                        i for i, x in enumerate(reversed(light_state_bin)) if x == "1"
                    ]
                    # get decimal value of 1's
                    light_state_dec_pos = [2**i for i in light_state_bin_pos]
                    # get VehicleLightState of 1's
                    light_state = [
                        carla.VehicleLightState.values[i] for i in light_state_dec_pos
                    ]
        ...
        traffic_lights = self.get_nearby_object(
            ego_transform.location,
            self._actors.filter("*light*"),
            self.config.bb_save_radius,
        )
        ...
        for traffic_light in traffic_lights:
            ...
            if not lidar is None:
                num_in_bbox_points = self.get_points_in_bbox(
                    relative_pos, relative_yaw, traffic_light_extent, lidar
                )
            else:
                num_in_bbox_points = -1
            ...
            traffic_light_wp = self.world_map.get_waypoint(
                trigger, project_to_road=False, lane_type=carla.libcarla.LaneType.Any
            )
        ...
        for stop_sign in stop_signs:
            ...
            if not lidar is None:
                num_in_bbox_points = self.get_points_in_bbox(
                    relative_pos, relative_yaw, stop_sign_extent, lidar
                )
            else:
                num_in_bbox_points = -1
            ...
            stop_sign_wp = self.world_map.get_waypoint(
                trigger, project_to_road=False, lane_type=carla.libcarla.LaneType.Any
            )
        ...
        for static in statics:
            if (
                ...
                if static.type_id == "static.prop.mesh":
                    ...
                    static.type_id == "static.prop.trafficwarning"
                ):  # the huge traffic warning sign in the scenarios ConstructionObstacle and ConstructionObstacleTwoWays
                    result = {
